...
| <a name="input_custom_sg_rules"></a> [custom\_sg\_rules](#input\_custom\_sg\_rules) | Custom security group rules | <pre>list(object({<br/>    key         = string<br/>    type        = string<br/>    from_port   = number<br/>    to_port     = number<br/>    protocol    = string<br/>    cidr_blocks = list(string)<br/>    description = string<br/>  }))</pre> | `[]` | no |
